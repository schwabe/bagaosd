/******* STARTUP PANEL *******/
/*
const int16_t BATTERY_MAH_CAPA[] = {0,3000,5000,8000};
int16_t LIPO_CAPACITY_MAH = 0;
int8_t battery_index = 0;
boolean change_battery_type = false;
*/
...
void writePanels(){ 
    ...
    if(localtime < (lastMAVBeat + 2200)){
    
        //DMD : Disable Waiting for MAVLink panel
        ...
        //DMD - Flight Time
        if( osd_throttle > 10 ) { //Flight time when there's some throttle
            flight_time += (localtime - last_time);
        }
        
        // If throttle percent above 95%, then we can change battery time (BATTERY_MAH_CAPA)
        // But flight time must be less than 5s, battery time no being changed and we need to display then panel for battery, otherwise it's useless
        /*
        if( osd_throttle > 95 &&  flight_time < 5000 && !change_battery_type && ISa(panel,Bp_BIT)) {
          battery_index = battery_index++ % (sizeof(BATTERY_MAH_CAPA) / 2);
          LIPO_CAPACITY_MAH = BATTERY_MAH_CAPA[battery_index];
        }
        
        if( change_value && osd_throttle < 5 ) {
          flight_time = 0; //Reset flight time
          change_battery_type = false;
        }
        */
...
void panRSSI(int first_col, int first_line){
    osd.setPanel(first_col, first_line);
    osd.openPanel();
    //rssical : RSSI Max value
    //rssipersent : RSSI Min value
...
void panCALLSIGN(int first_col, int first_line){
    ...
    //osd.printf("%s", char_call); 
    /*if (osd_throttle < 10 ) {
        osd.printf("%02i%c%02i%c%02i",((int)gps_utc_time_second/3600)%3600,0x3A,((int)gps_utc_time_second/60)%60,0x3A,(int)gps_utc_time_second%60);
    }*/ 
    osd.closePanel();
}
...
void panClimb(int first_col, int first_line){
    osd.setPanel(first_col, first_line);
    osd.openPanel();
    osd.printf("%c%3.0f%c",0x16, (double)(osd_climb * converts + 0.5f) , 0x81); //DMD : Km/h instead of m/s
    osd.closePanel();
}
...
void panBatteryPercent(int first_col, int first_line){
    ...
    //If battery value different than 0, we display percentage otherwise we display mah consumption
    /*if( battery_index > 0 ) {
      //Display battery type
      if( flight_time < 5000 ) {
        osd.printf("%c%4i%c", osd_battery_pic_A, BATTERY_MAH_CAPA[battery_index], 0x82); 
        //Then display percentage
      } else {
        float capacity = constrain(LIPO_CAPACITY_MAH - osd_battery_remaining_A, 0, LIPO_CAPACITY_MAH);
        
        osd.printf("%c%4i%c", osd_battery_pic_A, (int) (capacity/LIPO_CAPACITY_MAH*100.0), 0x82); 
      }
    } else {
      //Display consumtpion
      osd.printf("%c%4i%c", osd_battery_pic_A, (int)osd_battery_remaining_A, 0x82); 
    }*/
...
void panTime(int first_col, int first_line){
    ...
    start_Time = flight_time/1000;
    
    //Modif DMD
    //osd.printf("%c%2i%c%02i", 0xB3,((int)start_Time/60)%60,0x3A,(int)start_Time%60);
    osd.printf("%c%02i%c%02i", 0xB3,((int)start_Time/60)%60,0x3A,(int)start_Time%60);
    osd.closePanel();
}
